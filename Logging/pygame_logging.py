...
class BlueBlob(Blob):
    ...
    def __add__(self, other_blob):
        logging.info('Blob add op: {} + {}'.format(str(self.color), str(other_blob.color)))                                 #Logging!
    ...
    for blue_id, blue_blob in blues.copy().items():
        for other_blobs in blues, reds, greens:
            for other_blob_id, other_blob in other_blobs.copy().items():
                logging.debug('Checking if blobs touching {} + {}'.format(str(blue_blob.color), str(other_blob.color)))             #Logging!
